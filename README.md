...
Battle starts immediately when the game launches. Initially, two players are given a position in the map, player 1 to the left side and player 2 to the right side. The map consists of some barriers in which the rockets that come out of the tank can not pass. Instead, they will bounce back and after some seconds they will disappear. Also, the tank is not allowed to go through the barriers, instead it needs to go back or rotate, since the wall will block it from moving.
...
The **`btnLogin_Click`** is more complicated. Here, we look through the database and we make sure that the user with those credentials he just entered exists on the database so he will be logged in successfully. The game won't be accessible to play if there are not two users that are logged in. After two successfully logins, the game is ready to play with pressing the `ENTER` or `SPACE` button. With pressing one of these, the game will start with those two respective players. In this function I get the two users and their respective points as arguments, and after that I had to search the database to find those two players and their points. 

__After the game is finished__, I update their points by adding the existing points and the new points they just earned while playing. Before that, I make sure that the game is played only with registered users. If they are not registered, I just **`return`** and don't open the database for further changes.

## **Tank Features**

One another important feature or something that is characteristic for this tank is that it can rotate in 360 degrees, to the left and right. Tank also moves forward and backward.
...
Turret also shows us the bullets direction during a specific moment by firing bullets in certain area, which means bullets have the same direction turret has.
If the bullets face a barrier or another obstacle that prevents their movement or access, bullets can change the direction and continue moving in the opposite direction.
...
Now we are going to present some of project's code snippets in order to convey our approach that we used to make our idea a reality.
# Wall
